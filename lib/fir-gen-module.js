...
const flatTree = tree => {
  const res = [];
  const rec = (node, path) => {
    const pathNew = path.concat(node.name);
    // console.log(pathNew);
    if (node.children.length === 0) {
      res.push({
        type: 'input',
        name: pathNew.slice(1).join('.'),
        width: node.width,
        signed: node.signed,
        usage: 0
      });
...
//       type: 'inst',
//       instName: 'inst_' + i,
//       mode: mods[rnd.integer(0, mods.length - 1)(mt)]
//     });
//   }
// };
...
const genBody = (mt, opt, mods) => {
  ...
  let inputs;
  do {
    inputs = genInputTree(mt, opt);
    // nodes = genInputs(mt, opt);
    nodes = flatTree(inputs);
    // genInsts(mt, mods, nodes, opt); // 5.12

...
const emitSmem = node => {
  const res = [
    'smem ' + node.name + ' : ' + intw(node) + '[' + node.depth + ']'
  ];
  return res.join('\n');
};
...
const instInputer = mod => node => {
  const res = ['; ' + node.name];
  res.push(node.name + '.reset <= reset');
  res.push(node.name + '.arst <= arst');
  for (let i = 0; i < mod.numClocks; i++) {
    res.push(node.name + `.clock${i} <= clock${i}`);
  }
...
const wireEmitter = node => {
  ...
  case 'reg':  return ['reg' + ' ' + xint(node) + ', ' + node.clock];
  case 'regr': return ['reg' + ' ' + xint(node) + ', ' + node.clock + ' with: (reset => (reset, ' + intw(node) + '(0)))'];
  case 'rega': return ['reg' + ' ' + xint(node) + ', ' + node.clock + ' with: (reset => (arst, ' + intw(node) + '(0)))'];
  }
  return [];
};
...
module.exports = mod => [].concat(
  ...
  mod.insts.flatMap(instInputer(mod))
)
  .join('\n');

  ...
  if (opt === 'Xu') {
    ...
    if (args[1].width > 10) {
      ...
      args[1] = rnd.bool()(mt) ? {
        ...
        arguments: [
          args[1],
          {type: 'parameter', name: args[1].width - resw}
        ]
      } : {
        type: 'head', width: resw,
        arguments: [
          args[1],
          {type: 'parameter', name: resw}
        ]
      };
    }
...
const getOp = (mt, nodes) => {
  const opo = {
    ...
    pad: unoparam('pad', mt, (a, p) => Math.max(a.width, p), a => a.signed),
    ...
    dshl: binop('dshl', mt, a => a[0].width + Math.pow(2, a[1].width) - 1, a => a[0].signed, 'Xu'),
    dshr: binop('dshr', mt, a => a[0].width, a => a[0].signed, 'XU'),
